...
public final class Core extends AbstractNpcAI
{
	...
	public void spawnBoss(L2GrandBossInstance npc)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			final Location loc1 = GeoData.getInstance().moveCheck(npc.getX(), npc.getY(), npc.getZ(), x, 110000, npc.getZ(), npc.getInstanceId());
			mob = (L2Attackable) addSpawn(DEATH_KNIGHT, loc1.getX(), loc1.getY(), loc1.getZ(), 280 + getRandom(40), false, 0);
			mob.setIsRaidMinion(true);
			_minions.add(mob);
			
			final Location loc2 = GeoData.getInstance().moveCheck(npc.getX(), npc.getY(), npc.getZ(), x, 109000, npc.getZ(), npc.getInstanceId());
			mob = (L2Attackable) addSpawn(DEATH_KNIGHT, loc2.getX(), loc2.getY(), loc2.getZ(), 280 + getRandom(40), false, 0);
			mob.setIsRaidMinion(true);
			_minions.add(mob);
			
			final int x2 = 16800 + (i * 600);
			final Location loc3 = GeoData.getInstance().moveCheck(npc.getX(), npc.getY(), npc.getZ(), x2, 109300, npc.getZ(), npc.getInstanceId());
			mob = (L2Attackable) addSpawn(DOOM_WRAITH, loc3.getX(), loc3.getY(), loc3.getZ(), 280 + getRandom(40), false, 0);
			mob.setIsRaidMinion(true);
			_minions.add(mob);
		}
		for (int i = 0; i < 4; i++)
		{
			int x = 16800 + (i * 450);
			final Location loc1 = GeoData.getInstance().moveCheck(npc.getX(), npc.getY(), npc.getZ(), x, 110300, npc.getZ(), npc.getInstanceId());
			mob = (L2Attackable) addSpawn(SUSCEPTOR, loc1.getX(), loc1.getY(), loc1.getZ(), 280 + getRandom(40), false, 0);
			mob.setIsRaidMinion(true);
			_minions.add(mob);
		}
	}
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		if (npc.getId() == CORE)
		{
			...
			calendar.set(Calendar.HOUR_OF_DAY, Config.CORE_RESPAWN_HOUR);
			calendar.set(Calendar.MINUTE, Config.CORE_RESPAWN_MINUTE);
			calendar.set(Calendar.SECOND, 0);
			
			final List<Integer> daysOfWeek = Config.CORE_SPAWN_INTERVALS;
			...
			{
				calendar.add(Calendar.WEEK_OF_YEAR, Config.CORE_RESPAWN_WEEKS);
			...
			respawnTime += Rnd.get(0, getRandom(Config.CORE_SPAWN_RANDOM) * 60 * 1000);
