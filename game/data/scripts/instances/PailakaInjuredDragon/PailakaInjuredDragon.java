...
public class PailakaInjuredDragon extends Quest
{
	...
	protected class LATANAS_WORLD extends InstanceWorld
	{
		protected int buff_counter = 5;
		protected boolean _hasDoneAnimation = false;
	}
	...
	private static void giveBuff(L2Npc npc, L2PcInstance player, int skillId, int level)
	{
		...
		LATANAS_WORLD world = (LATANAS_WORLD) InstanceManager.getInstance().getPlayerWorld(player);
		if (world != null)
		{
			world.buff_counter--;
		}
	...
	{
		LATANAS_WORLD world = (LATANAS_WORLD) InstanceManager.getInstance().getPlayerWorld(attacker);
		switch (npc.getId())
		{
			case LATANA:
				if (world != null)
