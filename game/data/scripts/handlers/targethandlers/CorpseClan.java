			...
			if (clan != null)
			{
				...
				for (L2ClanMember member : clan.getMembers())
				{
					...
					if (skill.getId() == 1254)
					{
						...
						else if (obj.isResurrectionBlocked())
						{
							continue;
						}
						else if (obj.isReviveRequested())
						{
							continue;
						}
						
						if (obj.isInsideZone(ZoneIdType.SIEGE))
						{
							final Siege siege = SiegeManager.getInstance().getSiege(obj);
							final boolean twWar = TerritoryWarManager.getInstance().isTWInProgress();
							
							final L2Clan clanobj = obj.getClan();
							...
							if ((siege != null) && siege.isInProgress())
							{
								...
								else if (siege.checkIsDefender(clanobj) && (siege.getControlTowerCount() == 0))
								{
									continue;
								}
								else if (siege.checkIsAttacker(clanobj) && (siege.getAttackerClan(clanobj).getNumFlags() == 0))
								{
									continue;
								}
								else
