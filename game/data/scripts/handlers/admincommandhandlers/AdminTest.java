...
public class AdminTest implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_serverontime"))
		{
			int timeSeconds = GameTimeController.getInstance().getServerRunTime();
			String timeResult = "";
			if (timeSeconds >= 86400)
			{
				timeResult = Integer.toString(timeSeconds / 86400) + " Day(s) " + Integer.toString((timeSeconds % 86400) / 3600) + " Hr(s)";
			}
			else
			{
				timeResult = Integer.toString(timeSeconds / 3600) + " Hr(s) " + Integer.toString((timeSeconds % 3600) / 60) + " Min(s)";
			}
			
			activeChar.sendMessage("Server online time: " + timeResult);
		}
		...
		else if (command.equalsIgnoreCase("admin_statistics"))
		{
			final String statistics = ObjectsStorage.getStats().toL2Html();
			activeChar.sendMessage(statistics);
			Log.info(statistics);
			// activeChar.sendDebugMessage(msg);
		}
		return true;
	}
