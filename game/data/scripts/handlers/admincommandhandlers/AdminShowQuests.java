		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			switch (val[0])
			{
				case "full":
				{
					...
					try (PreparedStatement req = con.prepareStatement("SELECT DISTINCT name FROM character_quests WHERE charId='" + ID + "' AND var='<state>' ORDER by name"))
					{
						req.execute();
				...
				case "name":
				{
					...
					try (PreparedStatement req = con.prepareStatement("SELECT var,value FROM character_quests WHERE charId='" + ID + "' and name='" + val[1] + "'"))
					{
						req.execute();
						try (ResultSet rs = req.getResultSet())
						{
