	...
	public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets)
	{
		...
		for (L2Object target : targets)
		{
			...
			else if (target instanceof L2ChestInstance)
			{
				...
				if ((chest.getCurrentHp() <= 0.0) || (activeChar.getInstanceId() != chest.getInstanceId()))
				{
					activeChar.sendActionFailed();
					return;
				}
				final L2ChestInstance targ = (L2ChestInstance) target;
				if (!targ.isDead())
				{
					targ.tryOpen((L2PcInstance) activeChar, skill);
				}
			}
		}
	}
