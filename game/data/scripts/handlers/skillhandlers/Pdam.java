...
public class Pdam implements ISkillHandler
{
	...
	public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets)
	{
		...
		for (L2Object trg : targets)
		{
			...
			{
				damage = skill.isStaticDamage() ? (int) skill.getPower() : (SkillFormulas.calculateInt(FormulaType.CALC_PHYS_SKILL_DMG, activeChar, target, skill, new FormulaEnv(shld, ss, false, false, crit)));
			}
			...
			if (!SkillFormulas.calcPhysicalSkillEvasion(activeChar, target, skill))
			{
				if (skill.hasEffects())
				{
					...
					skill.getEffectsVoid(activeChar, target, new Env(shld, false, false, false));
				}
